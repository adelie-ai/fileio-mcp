...
pub fn rmdir_single(path: &str, recursive: bool) -> Result<()> {
    ...
    if !recursive {
        let mut entries = std::fs::read_dir(&expanded_path).map_err(|e| {
            crate::error::FileIoMcpError::from(FileIoError::ReadError(format!(
    ...
    if let Some(r) = results.get(0) {
        if r.status == "ok" {
            ...
            Err(crate::error::FileIoMcpError::from(FileIoError::WriteError(
                format!("Removal failed: {}: {}", r.path, r.status),
            )))
        }
    } else {
        Err(crate::error::FileIoMcpError::from(FileIoError::WriteError(
            "No result from rm".to_string(),
        )))
    }
}
